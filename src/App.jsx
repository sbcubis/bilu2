...
import { transcribeAudio, chat, speakText } from './api.js'
...
export default function App() {
  ...
  const [memoryLimit, setMemoryLimit] = useState(() =>
    parseInt(localStorage.getItem(LS_MEM) || CONFIG.DEFAULT_MEMORY_LIMIT)
  )
  const [voiceEnabled, setVoiceEnabled] = useState(() =>
    localStorage.getItem(LS_VOICE) !== 'false'
  )
  ...
  const handleSend = useCallback(async (text) => {
    ...
    // Build message list for OpenAI — include rolling history
    const newUserMsg = { role: 'user', content: text, ts: Date.now() }
    const updatedMessages = [...messages, newUserMsg].slice(-(memoryLimit * 2))
    setMessages(updatedMessages)

    // Format for OpenAI (strip ts field)
    const openAiMessages = updatedMessages.map(({ role, content }) => ({ role, content }))
    ...
    isHoldingRef.current = false
  }, [messages, voiceEnabled, memoryLimit])
  ...
  const handleFinalTranscript = useCallback(async (blob, liveText) => {
    ...
    if (blob && !CONFIG.OPENAI_API_KEY.startsWith('REPLACE')) {
      try { finalText = (await transcribeAudio(blob)) || liveText } catch (e) { console.warn('Whisper:', e.message) }
    }
    ...
    <div className="h-full relative bg-gradient-to-b from-background to-background/80 select-none"
        ...
        <div className="flex flex-col h-full">
          <div className="border-b border-border bg-card/30 backdrop-blur-sm px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-base font-display font-bold text-foreground">BILU</h1>
                <p className="text-[10px] text-muted-foreground/70">Online · Ready to assist</p>
              </div>
            </div>
            <button onClick={() => setIsOpen(false)}
              className="w-8 h-8 rounded-full bg-white/5 border border-border/50 flex items-center justify-center text-muted-foreground/50 hover:text-foreground transition-all">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-8 space-y-4">
            ...
            {messages.map((m, idx) => {
              ...
              return (
                <ChatMessage key={idx} message={m} assistantName="BILU"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6"
          ...
          <div className="w-full max-w-sm bg-card/95 backdrop-blur-xl border border-border/60 rounded-3xl p-6 shadow-2xl">
            ...
            <div className="space-y-5">
              ...
              <button onClick={() => { setMessages([]); localStorage.removeItem(LS_MSGS); setShowSettings(false) }}
      ...
      <div className="h-full flex items-center justify-center">
        <div className="relative">
          ...
          <div className={cn('absolute inset-0 rounded-full border-2 transition-all duration-300',
          ...
          <div className={cn('absolute inset-0 rounded-full border transition-all duration-300',
      ...
      <div className="absolute bottom-6 left-0 right-0 px-5 pointer-events-auto">
        <div className="flex items-end gap-2 bg-white/5 border border-white/10 rounded-2xl px-4 py-3 focus-within:border-primary/30 focus-within:ring-1 focus-within:ring-primary/20 transition-all">
          <textarea
            ...
            onKeyDown={e => {
              if (e.key==='Enter' && !e.shiftKey) {
                ...
                if (t && !isThinking) { setTextInput(''); handleSend(t) }
              }
            }}
          ...
          />
          <button
            onClick={() => { const t=textInput.trim(); if(t&&!isThinking){setTextInput('');handleSend(t)} }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6"
          ...
          <div className="w-full max-w-md flex flex-col bg-card/95 backdrop-blur-xl border border-border/60 rounded-3xl shadow-2xl shadow-black/60 overflow-hidden" style={{ maxHeight:'80vh' }}>
            ...
            <div className="px-6 pb-6 pt-4 border-t border-border/40 flex gap-3 flex-shrink-0">
              ...
              {isEditingReview ? (
                <button onClick={() => { const t=pendingTranscript.trim(); if(t) handleSend(t); setShowReviewModal(false);setPendingTranscript('');setIsEditingReview(false) }}
